...
class ProbabilisticPredictor(ABC):
    ...
    def visualize_inference(self, inputs, results, savedir, name, cfg, energy_threshold=None):
        ...
        box_features = results.box_features[0:max_boxes]
        if energy_threshold:
            # labels[(np.argwhere(
            #     torch.logsumexp(inter_feat[:, :-1], dim=1).cpu().data.numpy() < energy_threshold)).reshape(-1)] = 8
            
            # labels[(np.argwhere(
            #     self.model.roi_heads.logistic_regression(self.log_sum_exp(inter_feat[:, :-1], dim=1).view(-1,1)).argmax(dim=1).cpu().data.numpy() == 0
            # )).reshape(-1)] = 8    
            
            labels[(np.argwhere(
                self.model.roi_heads.logistic_regression(
                    self.log_sum_exp(
                        self.model.roi_heads.box_predictor(
                            self.model.roi_heads.similarity_mlp(box_features))[0][:, :-1], dim=1).view(-1,1)).argmax(dim=1).cpu().data.numpy() == 0
            )).reshape(-1)] = 8
    ...
    def log_sum_exp(self, value, dim=None, keepdim=False):
        ...
        if dim is not None:
            ...
            return m + torch.log(torch.sum(
                torch.nn.functional.relu(self.model.roi_heads.weight_energy.weight) * torch.exp(value0),dim=dim, keepdim=keepdim))
        else:
            m = torch.max(value)
