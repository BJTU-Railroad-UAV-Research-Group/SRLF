...
class GeneralizedRcnnPlainPredictor(ProbabilisticPredictor):
    ...
    def generalized_rcnn_probabilistic_inference(self,
        ...
        if is_epistemic:
            ...
            # box_features = self.model.roi_heads.box_pooler(features, [x["instances"].gt_boxes.to(features[0].device) for x in input_im])
            ...
            if len(predictions) == 3:
                raw_output.update({'box_cls_agnostic': predictions[2]})
            outputs = raw_output
            ####
        ...
        # box_cls_agnostic = outputs['box_cls_agnostic']
        box_features = outputs['box_features']

        inter_feat = box_cls
        box_cls = torch.nn.functional.softmax(box_cls, dim=-1)
        # box_cls_agnostic = torch.nn.functional.softmax(box_cls_agnostic, dim=-1)
        ...
        return boxes, boxes_covars, scores, inter_feat, filter_inds[:,
                                                        1], box_cls[filter_inds[:, 0]], det_labels, box_features
    ...
    def post_processing_standard_nms(self, input_im):
        ...
        return inference_utils.general_standard_nms_postprocessing(
            input_im, outputs, self.test_nms_thresh, self.test_topk_per_image, agnostic=True)
