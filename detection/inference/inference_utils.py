...
def general_standard_nms_postprocessing(input_im,
    ...
    try:
        ...
        classes_idxs, predicted_prob_vectors, det_labels, box_features = outputs
    except:
        predicted_boxes, predicted_boxes_covariance, predicted_prob, inter_feat, logistic_score, \
        classes_idxs, predicted_prob_vectors, det_labels, box_features = outputs
    _nms_threshold = 0.5 if agnostic else nms_threshold
    _classes_idxs = classes_idxs * 0 if agnostic else classes_idxs
    # Perform nms
    keep = batched_nms(
        predicted_boxes,
        predicted_prob,
    ...
    result.box_features = box_features[keep]
...
def instances_to_json_align_with_visualize(predictor, instances, img_id, cat_mapping_dict=None, energy_threshold=9.882123):
    """
    Dump an "Instances" object to a COCO-format json that's used for evaluation.
    ...
    # inter_feat = instances.inter_feat
    box_features = instances.box_features
    
    labels = instances.pred_classes
    # labels[(np.argwhere(
    #     torch.logsumexp(inter_feat[:, :-1], dim=1).cpu().data.numpy() < energy_threshold)).reshape(-1)] = 8
    # labels[(np.argwhere(
    #     predictor.model.roi_heads.logistic_regression(predictor.log_sum_exp(inter_feat[:, :-1], dim=1).view(-1,1)).argmax(dim=1).cpu().data.numpy() == 0
    # )).reshape(-1)] = 8
    
    labels[(np.argwhere(
                predictor.model.roi_heads.logistic_regression(
                    predictor.log_sum_exp(
                        predictor.model.roi_heads.box_predictor(
                            predictor.model.roi_heads.similarity_mlp(box_features))[0][:, :-1], dim=1).view(-1,1)).argmax(dim=1).cpu().data.numpy() == 0
            )).reshape(-1)] = 8
    ...
    classes = [
        cat_mapping_dict[class_i] if class_i in cat_mapping_dict.keys() else 1000 for class_i in classes]
    # breakpoint()
    pred_cls_probs = instances.pred_cls_probs.cpu().tolist()
