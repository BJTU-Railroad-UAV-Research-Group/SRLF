...
from detectron2.data.build import build_detection_train_loader1, build_detection_test_loader1
...
from inference.inference_utils import instances_to_json, get_inference_output_dir, build_predictor, instances_to_json_align_with_visualize
...
def main(args):
    ...
    # test_data_loader = build_detection_test_loader1(cfg) # build_detection_train_loader1(cfg) # OOD dataset load
        ...
        with torch.no_grad():
            ...
            for idx, input_im in enumerate(test_data_loader):
                ...
                final_output_list.extend(
                    ...
                    #     input_im[0]['image_id'],
                    #     cat_mapping_dict)
                    instances_to_json_align_with_visualize(
                        predictor,
                        outputs,
                        input_im[0]['image_id'],
                        cat_mapping_dict))
                try:
                    if idx + 1 == test_data_loader.__len__():
                        break
                except:
                    if idx + 1 == test_data_loader.dataset.dataset.dataset.__len__():
                        break

        # big_inference_output_dir = inference_output_dir
        big_inference_output_dir = args.savefigdir
